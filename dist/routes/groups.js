...
exports.ensureUserInDefaultGroups = ensureUserInDefaultGroups;
 ...
 */
async function ensureAdminsInGroup(groupId) {
    const admins = await database_1.db.user.findMany({
        where: { is_admin: 1 },
        select: { id: true }
    });
    for (const admin of admins) {
        await database_1.db.groupMember.upsert({
 ...
 */
async function ensureUserInDefaultGroups(userId) {
    ...
    const globalGroup = await database_1.db.group.upsert({
    ...
    if (user.school) {
        ...
        const schoolGroup = await database_1.db.group.upsert({
    ...
    if (user.grade) {
        ...
        const gradeGroup = await database_1.db.group.upsert({
...
router.get('/', posts_1.authenticate, async (req, res) => {
    try {
        ...
        const memberships = await database_1.db.groupMember.findMany({
        ...
        const groups = memberships.map(m => (Object.assign(Object.assign({}, m.group), { memberCount: m.group._count.members })));
        res.json(groups);
    }
    catch (error) {
...
router.get('/:groupId/messages', posts_1.authenticate, async (req, res) => {
    try {
        ...
        const messages = await database_1.db.groupMessage.findMany({
...
router.post('/:groupId/messages', posts_1.authenticate, async (req, res) => {
    try {
        ...
        // 2. Create message
        const message = await database_1.db.groupMessage.create({
            data: {
                group_id: groupId,
                user_id: userId,
                text
